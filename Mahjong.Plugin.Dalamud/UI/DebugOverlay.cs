...
public sealed class DebugOverlay : Window, IDisposable
{
    ...
    public override void Draw()
    {
        ...
        if (ImGui.BeginTabBar("##debug-tabs"))
        {
            ...
            { ImGui.Dummy(new Vector2(0, 4)); DrawStatusTab(); ImGui.EndTabItem(); }
            if (ImGui.BeginTabItem("Addon"))
            { ImGui.Dummy(new Vector2(0, 4)); DrawAddonTab(); ImGui.EndTabItem(); }
            if (ImGui.BeginTabItem("Actions"))
            { ImGui.Dummy(new Vector2(0, 4)); DrawActionsTab(); ImGui.EndTabItem(); }
            if (ImGui.BeginTabItem("Diagnostics"))
            { ImGui.Dummy(new Vector2(0, 4)); DrawDiagnosticsTab(); ImGui.EndTabItem(); }
            ImGui.EndTabBar();
        }
    ...
    {
        if (!cfg.TosAccepted)
            return (Theme.Danger, "TOS");
        if (cfg.AutomationArmed && !cfg.SuggestionOnly)
            return (Theme.Accent, "AUTO");
        if (cfg.AutomationArmed && cfg.SuggestionOnly)
            return (Theme.Warn, "HINTS");
    ...
    private void DrawStatusTab()
    {
        ...
        using (Theme.BeginCard("status-flags"))
        {
            ...
            if (ImGui.Checkbox("Automation armed", ref armed))
                plugin.ConfigService.Update(c => c with { AutomationArmed = armed && c.TosAccepted });
        ...
        using (Theme.BeginCard("actions-auto"))
        {
            ...
            using (Disable(!ourTurn))
            {
                ...
                if (ImGui.Button("Run policy pick", new Vector2(w, 34)))
                    framework.RunOnFrameworkThread(() => TriggerAutoDiscard());
            }
            if (!ourTurn)
            {
    ...
    private void DrawDiagnosticsTab()
    {
        ...
        using (Theme.BeginCard("diag-dump"))
        {
            ...
            if (ImGui.Button("Dump memory (0x0, 0x2000)"))
                commandManager.ProcessCommand("/mjauto dumpmem 0x0 0x2000");
            ImGui.SameLine(0, 6);
            if (ImGui.Button("Dump AtkValues"))
                commandManager.ProcessCommand("/mjauto atkvalues");
            ImGui.SameLine(0, 6);
            if (ImGui.Button("Dump AgentEmj"))
                commandManager.ProcessCommand("/mjauto agent 0x1000");
        }
    }
